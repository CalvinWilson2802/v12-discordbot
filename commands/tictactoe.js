module.exports = {
    ...
    aliases: ['ttt'],
    cooldown: '2',
      description: "Gives a tictactoe game with buttons!",
      usage: "ztictactoe [user]",
    
      async run (client, message, args) {
         
         let opponent = message.mentions.members.first() || message.guild.members.cache.get(args[0])
            if(!opponent) return message.reply("Mention a user to be ur opponent <a:DS_MachoFroge:815173459105021982> ")
            ...
            async function tictactoe(m) {
                ...
                collector.on('collect', b => {
                    ...
                    if (Object.keys(Brgs).length == 0) return m.edit('It\'s a tie! <a:SC_OMG:849883753127346246>')
                    tictactoe(m)
                });
                collector.on('end', collected => {
                    if (collected.size == 0) m.edit(`<@!${Args.userid}> didn\'t react in time! (30s) <a:DS_peepoBye:826010495580504064>`)
                });
            }
        }
    }